...
RayCasterCameraPlugin::RayCasterCameraPlugin(const mjModel *m, mjData *d,
  ...
  std::string iteration_order = mj_getPluginConfig(m, instance, ray_attributes[5]);
  if (iteration_order.empty()) {
    ...
    mju_error("RayCasterCameraPlugin: iteration_order must be 'height_width' or 'width_height'");
  }
